...
public class DniColourRenderer {
    ...
    public static void render(Color color, Vector center, double radius) {
        ...
        float eyeRadius = 1.0F + (1.0F - HSB[1]) * 3.0F;

        boolean outerEyelid = false;
        boolean innerEyelid = false;
        boolean eye = false;
        if (HSB[1] > 0.3F && HSB[2] > 0.3F) {
            if (H <= 28 || H > 274) {
                int temp = (H + 85) % 360;
                eyelidRadius = MathHelper.abs(MathHelper.cos(((float) temp / 114.0F)) * radius * 4.0D / 3.0D);
                outerEyelid = true;
                innerEyelid = true;
            }
            else if (H <= 114) {
                int temp = H - 30;
                outerEyelid = true;
                eyelidRadius = MathHelper.abs(MathHelper.sin(((float) temp / 86.0F)) * radius * 4.0D / 3.0D);
                eye = true;
            }
            else if (H <= 274) {
                int temp = H - 114;
                innerEyelid = true;
                eyelidRadius = MathHelper.abs(MathHelper.cos(((float) temp / 160.0F)) * radius * 4.0D / 3.0D);
                eye = true;
            }
        }
        else {
            float mod = MathHelper.min(HSB[1], HSB[2]) / 0.3F;
            eyeRadius *= HSB[2] / 0.3F;
            if (H <= 28 || H > 274) {
                int temp = (H + 85) % 360;
                if (mod < 0.3F) {
                    eyelidRadius = MathHelper.abs(MathHelper.max(MathHelper.cos(((float) temp / 114.0F)), 1.0F - mod / 0.3F) * radius * 4.0D / 3.0D);
                    eye = true;
                }
                else {
                    eyelidRadius = MathHelper.abs(MathHelper.min(MathHelper.cos(((float) temp / 114.0F)), (mod - 0.3F) / 0.7F) * radius * 4.0D / 3.0D);
                    innerEyelid = true;
                }
                outerEyelid = true;
            }
                ...
                int temp = H - 30;
                outerEyelid = true;
                eyelidRadius = MathHelper.abs(MathHelper.max(MathHelper.cos(((float) temp / 86.0F)), 1.0F - mod) * radius * 4.0D / 3.0D);
                eye = true;
            }
            else if (H <= 274) {
                int temp = H - 114;
                innerEyelid = true;
                eyelidRadius = MathHelper.abs(MathHelper.max(MathHelper.cos(((float) temp / 160.0F)), 1.0F - mod) * radius * 4.0D / 3.0D);
                eye = true;
            }
        }
